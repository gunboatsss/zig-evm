//! Precompiles used by typical Solidity / Foundry unit tests.
//! `0x01` ecrecover, `0x02` sha256, `0x04` identity. Others stay empty accounts.
...
pub fn is_precompile(address: u256) bool {
    return address == ecrecover_address or address == sha256_address or address == identity_address;
}

pub fn gas_cost(address: u256, input_len: u32) u64 {
    const words = (@as(u64, input_len) + 31) / 32;
...
pub fn execute(address: u256, input: []const u8, out: []u8) error{OutputTooLarge}!u32 {
    return switch (address) {
